...
class shipcog(commands.Cog):
    ...
    async def ship_command(self, ctx, person1: Member, person2: Member = "author"):
        ...
        if result <= 9:
            await ctx.send(await SL.removeat(f"sorry but.. the ship between {person1.display_name} and {person2.display_name} is as likely as a mango tree growing on a glacier, giving it a {result} / 100"))
        elif result <= 19:
            await ctx.send(await SL.removeat(f"the ship between {person1.display_name} and {person2.display_name} doesn't seem particularly likely, scoring a {result} / 100"))
        elif result <= 29:
            await ctx.send(await SL.removeat(f"{person1.display_name} and {person2.display_name} is not going to be a thing, with them scoring {result} / 100 i can't see how that would ever work"))
        elif result <= 39:
            await ctx.send(await SL.removeat(f"{person1.display_name} and {person2.display_name} are certainly not made for eachother, and i would not expect them to ever become a serious thing, especially not with a score of {result} / 100"))
        elif result <= 49:
            await ctx.send(await SL.removeat(f"a ship between {person1.display_name} and {person2.display_name} seems like it *might* possible... {result} / 100"))
        elif result <= 59:
            await ctx.send(await SL.removeat(f"a relationship among {person1.display_name} and {person2.display_name} seems very plausible, above average even, scoring a {result} / 100"))
        elif result < 69:
            ...
            await ctx.send(await SL.removeat(f"{person1.display_name} and {person2.display_name} are practically made for eachother, i mean they literally scored a {result} / 100"))
        elif result <= 79:
