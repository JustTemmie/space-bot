...
class fun(commands.Cog):
    ...
    @commands.command(name="texttoimage", aliases = ["tti","imageapi"], brief = "takes a string of text and converts it to an image, don\'t ask")
    ...
    async def texttoimageapi(self, ctx, *, content):
        ...
        embed = discord.Embed(title=f"{ctx.author.display_name} just API\'d {content}",
    ...
    @commands.command(name="pot", aliases=['potter', 'otterpog'], brief="a referance to the best meme ever made")
    ...
    @commands.command(name="tenor", aliases=['gif'], brief="sends a gif from tenor using the given term")
    ...
    @commands.command(name="avatar", aliases=['pfp'], brief="sends the profile picture of a specified user")
    @cooldown(5, 10, BucketType.guild)
    ...
    @commands.command(name="mock", aliases=["taunt", "tease", "scoff", "makefunof"], brief="bully a user, anyone, not me though, please - also jovi, fuck you <3 (platonically)")
    ...
    @commands.command(name="alientohuman", aliases=["ath"], brief="translates alien characters into latin ones")
    ...
    async def translatealien(self, ctx, *, input=None):
        ...
        output = ''
        for ch in input: output += alien_to_human.get(ch.lower(), ch)
    ...
    @commands.command(name="humantoalien", aliases=["hta"], brief="translates the latin alphabet into alien")
    @cooldown(10, 45, BucketType.user)
    async def translatehuman(self, ctx, *, input=None):
        ...
        output = ''
        for ch in input: output += human_to_alien.get(ch.lower(), ch)
    ...
    @commands.command(name="morse", aliases=["htm","humantomorse"], brief="translates the latin alphabet into morse code")
    @cooldown(10, 45, BucketType.user)
    ...
    @commands.command(name="fact", aliases=["info", "animal"], brief="Tells you a random fact about the specified animal", description="The list of animals you can ask facts about are, dog, cat, panda, fox, bird, koala")
    @cooldown(3, 5, BucketType.guild)
        ...
        if (animal := animal.lower()) in ("dog", "cat", "panda", "fox", "bird", "koala"):
            ...
            async with request("GET", fact_url, headers={}) as response:
                if response.status == 200:
                    ...
                    embed.add_field(name = "note", value = "these are taken from the internet, take them with a grain of salt")
                    await ctx.send(embed=embed)
    ...
    @commands.command(name="dice", aliases=["roll"], brief="Roll some virtual dice! dice <amount of dice>d<wanted sides on dice>")
