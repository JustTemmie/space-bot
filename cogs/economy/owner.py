...
class ecoowner(commands.Cog):
    ...
    @commands.command(name="ownergive", aliases=["owner_give"])
    @commands.is_owner()
    async def ownerGiveCommand(self, ctx, member: Member, amount: int):
        ...
        await ctx.send(await SL.removeat(f"god gave {amount} <:beaverCoin:1019212566095986768> to {member.display_name}"))
