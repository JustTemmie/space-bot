...
@courtBot.command()
async def help(context, input = "asd"):
    if input != "render":
        return
    dummyAmount = random.randint(2, 150)
    helpEmbed = discord.Embed(title = f"This command is not made by the same developers as the rest of the bot\nfind their project here (https://github.com/LuisMayo/ace-attorney-discord-bot)", color=0x3366CC, footer="Do not include these symbols (\"<\" and \">\") when using this command")
...
async def on_ready():
    ...
    await courtBot.change_presence(status=discord.Status.idle)
